...
const plan = {
  input_token_price_micro: 10,
  cached_input_token_price_micro: 3,
  output_token_price_micro: 20
};
...
test('token quota quantity includes all token categories', () => {
  assert.equal(totalAiTokens({ inputTokens: 100, cachedInputTokens: 20, outputTokens: 30, reasoningTokens: 10 }), 160);
});
